...
const UpcomingPremieres = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/movie/upcoming')
      ...
      .catch(err => {
        console.error('Ошибка загрузки премьер:', err);
        setError('Не удалось загрузить список премьер. Попробуйте позже.');
      });
  }, []);
  ...
  return (
    <div className="p-4">
      {error && (
        <div className="bg-red-100 text-red-700 p-3 mb-4 rounded-lg text-center font-semibold shadow">
          {error}
        </div>
      )}
