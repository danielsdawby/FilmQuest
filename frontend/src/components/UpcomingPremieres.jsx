...
const UpcomingPremieres = () => {
  ...
  const [trailer, setTrailer] = useState(null); // Состояние для трейлера
  ...
  const closeModal = () => {
    ...
    setTrailer(null); // Очистка трейлера
  };

  const handlePosterClick = (movieId) => {
    // Перенаправление на страницу фильма по его ID
    navigate(`/details/${movieId}`); // Это будет ссылка вида http://localhost:5173/details/{movieId}
  };
            ...
            <div className="min-w-[350px] max-w-[450px] rounded-xl bg-gray-900 text-white shadow-lg overflow-hidden">
              <div className="relative">
                <img
                  ...
                  onClick={() => handlePosterClick(movie.id)} // Клик по постеру
                />
                <button
